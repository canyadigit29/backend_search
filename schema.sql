...
CREATE OR REPLACE FUNCTION "public"."match_documents"("query_embedding" "public"."vector", "match_threshold" double precision, "match_count" integer, "user_id_filter" "uuid", "filter_document_type" "text" DEFAULT NULL::"text", "filter_start_date" "date" DEFAULT NULL::"date", "filter_end_date" "date" DEFAULT NULL::"date") RETURNS TABLE("id" "uuid", "file_id" "uuid", "content" "text", "page_number" integer, "score" double precision)
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    RETURN QUERY
...
ALTER FUNCTION "public"."update_document_chunks_tsv"() OWNER TO "postgres";
...
COMMENT ON COLUMN "public"."files"."ocr_scanned" IS 'Indicates whether the file has been processed by OCR.';



COMMENT ON COLUMN "public"."files"."ocr_started_at" IS 'Timestamp for when OCR processing began.';



COMMENT ON COLUMN "public"."files"."ocr_completed_at" IS 'Timestamp for when OCR processing completed.';



COMMENT ON COLUMN "public"."files"."ocr_needed" IS 'Indicates if the file is a candidate for OCR processing.';



COMMENT ON COLUMN "public"."files"."ocr_text_path" IS 'Path to the stored text file after OCR.';
...
CREATE OR REPLACE TRIGGER "tsvector_update" BEFORE INSERT OR UPDATE ON "public"."document_chunks" FOR EACH ROW EXECUTE FUNCTION "public"."update_document_chunks_tsv"();
...
CREATE POLICY "Allow service roles to bypass RLS" ON "public"."files" USING (((("current_setting"('request.jwt.claims'::"text", true))::"jsonb" ->> 'role'::"text") = 'service_role'::"text"));



CREATE POLICY "Allow service roles to bypass RLS for chunks" ON "public"."document_chunks" USING (((("current_setting"('request.jwt.claims'::"text", true))::"jsonb" ->> 'role'::"text") = 'service_role'::"text"));



CREATE POLICY "Allow users to manage their own files" ON "public"."files" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Allow users to read their own document chunks" ON "public"."document_chunks" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
GRANT ALL ON FUNCTION "public"."update_document_chunks_tsv"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_document_chunks_tsv"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_document_chunks_tsv"() TO "service_role";
